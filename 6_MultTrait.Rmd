...
```{r warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.pos="H", fig.cap="Box Plot for grain yield (GY), date of maturity (DM), Plant height (PH), and Lodging (LD) across environment"}
...
```{r TDscatter gy, echo=FALSE, fig.align="center", fig.height=figheight, fig.width=figwidth, message=FALSE, fig.pos="H", warning=FALSE, dpi=600, fig.cap="Scatterplot matrix of trials for grain yield (GY). The diagonal shows the distribution and the bottom left shows the scatter points for DM in each location."}
...
```{r TDscatter dm, echo=FALSE, fig.align="center", fig.height=figheight, fig.width=figwidth, message=FALSE, fig.pos="H", warning=FALSE, dpi=600, fig.cap="Scatterplot matrix of trials for days of maturity (DM). The diagonal shows the distribution and the bottom left shows the scatter points for DM in each location."}
...
```{r TDscatter ph, echo=FALSE, fig.align="center", fig.height=figheight, fig.width=figwidth, message=FALSE, fig.pos="H", warning=FALSE, dpi=600, fig.cap="Scatterplot matrix of trials for plant height (PH). The diagonal shows the distribution and the bottom left shows the scatter points for PH in each location."}
...
```{r TDscatter ld, echo=FALSE, fig.align="center", fig.height=figheight, fig.width=figwidth, message=FALSE, fig.pos="H", warning=FALSE, dpi=600, fig.cap="Scatterplot matrix of trials for lodging (LD). The diagonal shows the distribution and the bottom left shows the scatter points for LD in each location."}
...
```{r missing geno gy all traits, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", fig.pos="H", dpi = 600, fig.cap="Missing genotypes by market classes"}
...
```{r corrbb1, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.pos="H", fig.cap="Pearson’s linear correlation between the studied traits. The lower diagonal shows the scatter plot where the environments are mapped with diferent point colors (BA = green, SA = salmon, TU = cyan)"}
...
```{r  warning=FALSE, fig.pos="H", message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.cap="Cultivars ranking based on the multi-trait stability index. Selected cultivars are highlighted in red."}
...
blup.mtme.bb<- as.data.frame((mod_MTME.bb$predictions$pvals[1:6])) # set the BLUP 
...
# results as data.table
blup.mtme.nb<- as.data.frame((mod_MTME.nb$predictions$pvals[1:6])) # set the BLUP
...
### MTME COMBINED ANALYSIS - COMB_MKT
...
```{r warning=FALSE, message=FALSE, results='asis'}
data_beans_all <- rbind(data_beans_BB, data_beans_NB, data_beans_SR)

mod_MTME.all <- asreml(fixed  = cbind(GY, DM, PH, LD) ~ trait:rep + trait, # trait:rep -1,
                      random =  ~ us(trait):name + us(trait):name:loc, #name:us(loc)
                      #residuals =  ~dsum(~us(name)|loc),
                      residuals = ~ units:us(trait),
                      data = data_beans_all,
...
#update.asreml(mod_MTME.bb)
d1<- data.frame(wald(mod_MTME.all))


d2<- data.frame(summary.asreml(mod_MTME.all)$varcomp) 

# > data.frame(summary.asreml(mod_MTME.all)$varcomp) 
#                            component std.error z.ratio bound X.ch
# trait:name!trait_GY:GY      0.237105    0.0485   4.886     P    0
# trait:name!trait_DM:GY      0.102360    0.0404   2.531     P    0
# trait:name!trait_DM:DM      0.300280    0.0612   4.905     P    0
# trait:name!trait_PH:GY      0.094403    0.0348   2.714     P    0
# trait:name!trait_PH:DM      0.073377    0.0373   1.968     P    0
# trait:name!trait_PH:PH      0.171194    0.0434   3.946     P    0
# trait:name!trait_LD:GY      0.027135    0.0417   0.651     P    0
# trait:name!trait_LD:DM      0.098660    0.0462   2.138     P    0
# trait:name!trait_LD:PH     -0.042950    0.0374  -1.148     P    0
# trait:name!trait_LD:LD      0.269085    0.0607   4.430     P    0
# trait:name:loc!trait_GY:GY  0.082926        NA      NA     B   NA
# trait:name:loc!trait_DM:GY -0.000281        NA      NA     B   NA
# trait:name:loc!trait_DM:DM  0.084023        NA      NA     B   NA
# trait:name:loc!trait_PH:GY -0.002016        NA      NA     B   NA
# trait:name:loc!trait_PH:DM -0.002670        NA      NA     B   NA
# trait:name:loc!trait_PH:PH  0.087605        NA      NA     B   NA
# trait:name:loc!trait_LD:GY  0.001092        NA      NA     B   NA
# trait:name:loc!trait_LD:DM -0.002484        NA      NA     B   NA
# trait:name:loc!trait_LD:PH  0.004115        NA      NA     B   NA
# trait:name:loc!trait_LD:LD  0.074118        NA      NA     B   NA
# units:trait!R               1.000000        NA      NA     F    0
# units:trait!trait_GY:GY     0.658078    0.0316  20.836     P    0
# units:trait!trait_DM:GY     0.056694    0.0238   2.381     P    0
# units:trait!trait_DM:DM     0.603377    0.0317  19.027     P    0
# units:trait!trait_PH:GY     0.151808    0.0267   5.678     P    0
# units:trait!trait_PH:DM    -0.001821    0.0249  -0.073     P    0
# units:trait!trait_PH:PH     0.751100    0.0392  19.154     P    0
# units:trait!trait_LD:GY     0.050337    0.0265   1.900     P    0
# units:trait!trait_LD:DM     0.087107    0.0251   3.465     P    0
# units:trait!trait_LD:PH    -0.014251    0.0274  -0.519     P    0
# units:trait!trait_LD:LD     0.677264    0.0368  18.393     P    0

# results as data.table
blup.mtme.all <- as.data.frame(matrix(unlist(mod_MTME.all$predictions$pvals[1:6]), ncol=6))
...
# > summary(Mod_GY)$varcomp
#          component std.error z.ratio bound %ch
# name        0.2375    0.0492    4.82     P   0
# name:loc    0.0815    0.0275    2.96     P   0
# units!R     0.6596    0.0332   19.86     P   0
# > summary(Mod_DM)$varcomp
#          component std.error z.ratio bound %ch
# name        0.3049    0.0625    4.88     P   0
# name:loc    0.0736    0.0285    2.58     P   0
# units!R     0.6061    0.0332   18.25     P   0
# > summary(Mod_PH)$varcomp
#          component std.error z.ratio bound %ch
# name        0.1877    0.0448    4.19     P   0
# name:loc    0.0381    0.0297    1.28     P   0
# units!R     0.7663    0.0421   18.22     P   0
# > summary(Mod_LD)$varcomp
#          component std.error z.ratio bound %ch
# name       0.28738    0.0607   4.734     P 0.1
# name:loc   0.00802    0.0245   0.328     P 0.0
# units!R    0.70090    0.0403  17.397     P 0.0
...
### Individual market bean class

- Variance Component and correlations analysis

```{r fig mtme corr0, fig.pos="H", warning=FALSE, out.width="100%", message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.cap="Genotypic correlation between GY, DPM, and PH across genotypes and locations for Black (BB) and Navy (NB) beans. The genotype (G) and genotype × environment interactions (GEI) correlation coefficients between the GY, DM, PH, and LD traits"}
# Black
#traits
...
ggplot(data = corr1, aes(x = names, y = corr, fill = names2, pattern = mkt)) +
  geom_bar_pattern(
    stat = "identity",
  ...
  scale_pattern_manual(values = c(BB = "none", NB = "stripe")) +
  scale_fill_manual(values = branded_colors) +
  ...
  labs(caption = NULL, x = NULL, y = "Rge", fill = 'Traits', pattern = "Market") +
  scale_y_continuous(limits = c(-0.8, 0.8), n.breaks = 10) +
  geom_vline(xintercept = 12.47) +
  annotate(geom = "text", x = 6.6, y = 0.8, label = "G", size = 5, fontface = "bold", color = "black") +
  annotate(geom = "text", x = 18.5, y = 0.8, label = "GxE", size = 5, fontface = "bold", color = "black") +
...
### Combining all market classes

```{r fig mtme corr1, fig.pos="H", warning=FALSE, out.width="100%", message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.cap="Genotypic correlation between GY, DPM, and PH across genotypes and locations for all market classes. The genotype (G) and genotype × environment interactions (GEI) correlation coefficients between the GY, DM, PH, and LD traits."}

#traits
a=summary(mod_MTME.all)$varcomp[1:10,1]
...
#int traits - locations
b=summary(mod_MTME.all)$varcomp[11:20,1]
...
corr=as.matrix(bdiag(cora,corb))
...
labs(caption=NULL, x=NULL, y="Rge", fill = 'Traits') +
  ...
  annotate(geom="text", x=3.3, y=0.55, label="G",size=5, fontface="bold",
           color="black") +
  annotate(geom="text", x=9.23, y=0.55, label="GxE",size=5, fontface="bold",
           color="black")
