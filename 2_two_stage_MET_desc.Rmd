...
if(y == 1) {next
  ...
  blue.1<- as.data.frame((mod.1$predictions$pvals[1:4])) 
...
```{r plot_ggstatsplot_met2, fig.height=figheight, fig.width=figwidth, message=FALSE, fig.pos= "H", out.width="100%", warning=FALSE, dpi=600, fig.cap="Combination of box and violin plots along with jittered data points for between subjects comparisons by locations of grain yield (GY) for the combined analysis (CombMKT) across all market classes. Pairwise Games-Howell test used. Comparisons showing only significant values. BA: Bay, HU: Huron, SA:Sanilac, TU: Tuscola."}

plotDM_stats_mkt_loc<- ggbetweenstats(data=blues_stage.I, x= loc, y=yield, type = "parametric",  bf.message = F, results.subtitle = F, 
                              ylab= "GY", xlab = "Locations",
                              plot.type = "boxviolin"
                              ) 
...
```{r MET metan, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.pos="H", fig.cap="Genotype’s performance across the environments for Black (BB), Navy (NB), and Small Red (SR) beans using the estimated means (BLUEs) from the 1-stage mixed model analysis. "}
...
```{r MET metan all mkt, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.pos="H", fig.cap="Genotype’s performance across the environments for all beans maket classes combined (combMKT)  using the estimated means (BLUEs) from the 1-stage mixed model analysis. "}

stab_allMKT <- ge_plot(blues_stage.I, type = 1,
        env = loc,
        gen = name,
        resp = yield) + xlab("") + ylab("Genotypes") +
theme(axis.text.x = element_text(size = 8, angle = 90, vjust = 0.5, hjust = 1),
...
Combined analysis across all market classes (Comb_MKT)

```{r}
mod.us.all <- asreml(fixed = yield ~ loc + loc:rep ,
                      random = ~ name:us(loc) ,
                      data = blues_stage.I,
...
f=summary(mod.us.all)$varcomp[1:10,1]
z=matrix(0, 4,4)
...
plotAll<- ggcorrplot(corf, colors = c("#6D9EC1", "white", "#E46726"),
title = " Comb_MKT",
show.legend = F,
...
```{r corr combined figure,  warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.pos="H"}
