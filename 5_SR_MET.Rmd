...
```{r mod met asreml sr, warning=FALSE, message=TRUE, results='asis'}
mod.met.asreml.sr1 <- asreml(fixed       = yield ~  loc + loc:rep,
...
summFix.sr.met.asreml<- data.frame(wald(mod.met.asreml.sr1))
summFix.sr.met.asreml

summ.sr.met.asreml<- data.frame(summary.asreml(mod.met.asreml.sr1)$varcomp)
summ.sr.met.asreml

#print(summary.asreml(mod.met.asreml.sr1)$bic)
mod.met.asreml.sr<- as.data.frame((mod.met.asreml.sr1$predictions$pvals[1:3]))
names(mod.met.asreml.sr) <- c("name", "yield_BLUPS_MET", "SE")
...
```{r mod metan sr, warning=FALSE, message=TRUE, results='asis'}
mixed_mod.sr<- 
   gamem_met(blues_stage.I_SR,
             env = loc,
             gen = name,
             rep = rep,
             resp = yield,
...
data_mod_sr_test <- get_model_data(mixed_mod.sr, "lrt")
data_mod_sr_test
...
```{r  warning=FALSE, message=TRUE, results='asis'}
data_mod_sr_det <- get_model_data(mixed_mod.sr, "details")
data_mod_sr_det
...
data_mod_sr_var <- get_model_data(mixed_mod.sr, "vcomp")
data_mod_sr_var
```
...
data_mod_sr_comp <- get_model_data(mixed_mod.sr)
data_mod_sr_comp
```
...
```{r sr gge fig1, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600}
gge_model.sr <- gge(blues_stage.I_SR, loc, name, yield, 
                    centering = "environment", #1
                    scaling = "sd", #2
                    svp = "environment")#2

a <- plot(gge_model.sr, type=4,
            size.text.env = 4.5,
...
```{r sr gge fig2, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600}
gge_model.sr <- gge(blues_stage.I_SR, loc, name, yield,
                     centering = "environment", #2
                    scaling = "sd", #1
                    svp = "genotype")#2)

e <- plot(gge_model.sr, type = 3,
          size.text.env = 5,
          plot_theme = theme_metan(grid =  "both",color.background = transparent_color()),
         axis_expand = 1.2,
         size.line = 0.7,
...
```{r mod metan sr stab, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600}
waasb_model_sr <- 
  waasb(blues_stage.I_SR,
        env = loc,
        gen = name,
        rep = rep,
        resp = yield,
...
waasb_model<- waasb_model_sr$yield$model

waasp_plot <- plot_scores(waasb_model_sr, type = 3,
          title = FALSE,
          size.tex.gen = 4,
          size.tex.env = 4,
          size.tex.lab = 13,
  ...
  geom_mark_rect(aes(filter =  Code  %in% c("R13", "R11", "R21", "R1"),
                     ),
               label.fontsize = 10,
               show.legend = F,
               con.cap = 0,
...
```{r sr selected waasb, warning=FALSE, message=TRUE, results='asis'}
...
```{r mod metan sr stab2, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600, fig.cap="Mean performance (a) and stability (b) for grain yield (GY) of 24 Small Red beans genotypes. The vertical dashed and solid lines shows, respectivelly, the  mean of the selected genotype and the overall mean for both mean performance and WAASB index"}

#Create a data frame with BLUPS - selected and non-selected
blups_sel <-
  gmd(waasb_model_sr, "blupge") %>%
...
```{r mod metan sr stab3_gain, warning=FALSE, message=TRUE, results='asis'}

blups_sel2 <-
  gmd(waasb_model_sr, "blupg") %>%
  add_cols(SELECTED = ifelse(GEN %in% selected$Code, "yes", "no")) %>% 
    dplyr::rename(BLUPs_sel = yield) %>% 
  droplevels()
...
```{r mod metan sr stab4, warning=FALSE, message=TRUE, results='asis'}
...
#write.xlsx(data_sel_perc, "./data/sel_SD_sr_2.xlsx")
...
```{r mod metan sr stab5, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600}
##BLUPs indexes
stab_blups_sr<- blup_indexes(waasb_model_sr) 
stab_blups_sr<- as_tibble(stab_blups_sr$yield)

data_waasby <- waasb_model_sr$yield$model %>% 
  dplyr::filter(type != "ENV") %>% 
  dplyr::select("Code", "WAASBY", "OrWAASBY") %>% 
  dplyr::rename(GEN = Code)

stab_blups_sr<- stab_blups_sr %>% 
  full_join(data_waasby, by = "GEN")


if (knitr::is_html_output()) {
  
  print_table(stab_blups_sr)
...
# library(openxlsx)
# write.xlsx(stab_blups_sr, "./data/blups_sr_2.xlsx")
...
```{r mod metan sr stab6, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600}
scenarios <- wsmp(waasb_model_sr,progbar = F)
...
```{r mod metan sr stab7, warning=FALSE, message=FALSE, fig.height=figheight, fig.width=figwidth, fig.align="center", dpi = 600}
coinc_1 <- stab_blups_sr %>% dplyr::select(GEN,HMRPGV_R) %>% arrange(HMRPGV_R)
coinc_2 <- stab_blups_sr %>% dplyr::select(GEN,RPGV_R) %>% arrange(RPGV_R)
coinc_3 <- stab_blups_sr %>% dplyr::select(GEN,HMGV_R) %>% arrange(HMGV_R)
coinc_4 <- stab_blups_sr %>% dplyr::select(GEN,OrWAASBY) %>% arrange(OrWAASBY)
coinc_5 <- stab_blups_sr %>% dplyr::select(GEN,WAASB_R) %>% arrange(WAASB_R)

selc_perc<- round(nrow(stab_blups_sr)*0.2)
...
plotSR<- ggcorrplot(z, colors = c("#6D9EC1", "gray" ,"#E46726"),  
           show.legend = T,
...
outline.color = "white",pch.col = "white", tl.col = "blue",show.diag = F) +
  labs(title = "BLUP-based stability indexes coincidence at SR",
           subtitle = "CSelection intensity of 20% top genotypes")
